...
// 🛠️ FIX: Set floorDropY to 600 (positive & larger)
const SCENE_LAYERS = [
  ...
  { id: 'clearing', name: 'Clearing', src: clearingImg, depth: 0.4, baseScale: 1.0, zIndex: 4, visible: true, slideOutX: 0, layerType: 'floor', floorDropY: 600 },
...
function Scene01CameraTest() {
  ...
  const getLayerTransform = useCallback((layer) => {
    ...
    if (layer.layerType === 'floor') {
      // 🛠️ FIX: Positive floorDropY so it slides down, but leaves ground at the bottom
  ...
  const handleZoomIn = () => {
    setCamera(prev => ({ ...prev, zoom: Math.min(prev.zoom + 0.1, 1.6) }));
  };
