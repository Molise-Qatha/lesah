...
// Layer configuration (all layers now have a `distance` for perspective)
const SCENE_LAYERS = [
  { id: 'sky', name: 'Sky', src: skyImg, zIndex: 1, visible: true, distance: 1000, baseScale: 1.0 },
  { id: 'mountains', name: 'Mountains', src: mountainsImg, zIndex: 2, visible: true, distance: 800, baseScale: 1.0 },
  { id: 'distant_forest', name: 'Distant Forest', src: forestImg, zIndex: 3, visible: true, distance: 600, baseScale: 1.0 },
  { id: 'clearing', name: 'Clearing', src: clearingImg, zIndex: 4, visible: true, distance: 400, baseScale: 1.0, floorDropY: 200 },
  { id: 'landmark_tree', name: 'Landmark Tree', src: landmarkImg, zIndex: 5, visible: true, distance: 200, baseScale: 1.0 },
  { id: 'near_tree_01', name: 'Near Tree 01', src: nearTree01Img, zIndex: 6, visible: true, distance: 100, baseScale: 1.15 },
  { id: 'near_tree_02', name: 'Near Tree 02', src: nearTree02Img, zIndex: 7, visible: true, distance: 50, baseScale: 1.25 },
];

const CAMERA_PATH = {
  startZ: 950, // Start far back
  endZ: 100,   // End between the trees, far into the forest
  duration: 14000,
};
...
function Scene01CameraTest() {
  ...
  // As camera.z decreases (moves forward), layers get closer and BIGGER naturally.
  const getLayerTransform = useCallback((layer) => {
    const distance = Math.max(layer.distance - camera.z, 10);
    
    // Perspective Scaling: 100 / distance
    const scale = (100 / distance) * layer.baseScale;

    // Perspective Parallax
    const parallaxX = camera.x * (100 / distance);
    const parallaxY = camera.y * (100 / distance) * 0.5;

    // Flat Floor
    let floorDrop = 0;
    if (layer.id === 'clearing') {
      floorDrop = (1 - (camera.z / CAMERA_PATH.startZ)) * layer.floorDropY;
    }
  ...
  useEffect(() => {
    ...
    const animateCamera = () => {
      ...
      setCamera(prev => ({ 
        ...prev, 
        z: newZ,
        x: Math.sin(eased * Math.PI) * 10 // Natural sway
      }));
    ...
    <div className="scene01-page">
      <div className="scene01-container">
        ...
        <div className="scene-header">
          ...
          <p className="scene-subtitle">True 3D Perspective Camera — Forest Dolly</p>
                ...
                >
                  ...
                  {debugMode && (
                    <div className="layer-debug-info">
                      ...
                      <span>Depth: {layer.distance}</span>
                      <span>Dist: {Math.max(layer.distance - camera.z, 10).toFixed(0)}</span>
                    </div>
                  )}
                </div>
              )
            ...
            <div className="debug-layers">
              ...
              {SCENE_LAYERS.map(layer => (
                <label key={layer.id} className="layer-toggle">
                  ...
                  <span className="depth-value">({layer.distance})</span>
                </label>
              ))}
            </div>
          )}
